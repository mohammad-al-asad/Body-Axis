...
export async function uploadIntroductionVideoMultipart(file, { onProgress } = {}) {
  const upload = await adminApi.initiateIntroductionVideoMultipartUpload({
    file_name: file.name,
  ...
  try {
    ...
    await adminApi
      .abortIntroductionVideoMultipartUpload({
        key: upload.key,
